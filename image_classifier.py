...
class Classifier:
    ...
    def AttentionModule(self, name="AttentionModule"):
        ...
        def spatial_attention(x):
            ...
            feature_map = layers.BatchNormalization()(feature_map)
            feature_map = layers.Activation("relu")(feature_map)
            attention_scores = layers.Conv2D(filters=1, kernel_size=5, padding='same', activation='sigmoid', kernel_initializer='he_normal', use_bias=False)(feature_map)
            return layers.multiply([x, attention_scores])
    ...
    def evaluate(self, X, Y, class_labels=None):
        ...
        for label_col in range(len(class_labels)):
            ...
            loss = log_loss(y_true_label, probs)
    ...
    def train_generator(self, augment_data=False):
        ...
        while True:
            sample_idxs = [np.random.choice(train_idx, self.batch_size, p=selection_method[i]) #self.selection_probs[i]
    ...
    def train(self, X: np.ndarray, Y: np.ndarray, max_epochs=400, test=False, test_set=None):
        ...
        history = self.model.fit(train_dataset, steps_per_epoch=100, epochs=max_epochs, 
                                 ...
                                 callbacks=[MergeEnsembleF1(self.ensemble_size), stopping, reduce, warmup], verbose=2).history
        self.save_model()
        if test:
    ...
    def write_results(self, result_data):
        ...
        with open("./results/{}_results.txt".format(self.name), 'w') as f:
            ...
            f.write("\nAverage Macro Loss: " + str(np.round(np.mean([results[0]["loss"] for results in result_data]), 3)))
            f.write("\nAverage Macro Accuracy: " + str(np.round(np.mean([results[0]["accuracy"] for results in result_data]), 3)))
            f.write("\nAverage Macro F1-Score: " + str(np.round(np.mean([results[0]["f1-score"] for results in result_data]), 3)))
            f.write("\nAverage Macro Precision: " + str(np.round(np.mean([results[0]["precision"] for results in result_data]), 3)))
            f.write("\nAverage Macro Recall: " + str(np.round(np.mean([results[0]["recall"] for results in result_data]), 3)))
            f.write("\nAverage AUC: " + str(np.round(np.mean([results[0]["auc"] for results in result_data]), 3)))
            for i, cl in enumerate(classes):
                f.write("\n\n" + cl + ':\n')
                f.write("\nAverage Binary Loss: " + str(np.round(np.mean(losses, axis=0)[i], 3)))
                f.write("\nAverage Binary Accuracy: " + str(np.round(np.mean(accs, axis=0)[i], 3)))
                f.write("\nAverage Binary F1-Score: " + str(np.round(np.mean(f1s, axis=0)[i], 3)))
                f.write("\nAverage Binary Precision: " + str(np.round(np.mean(prs, axis=0)[i], 3)))
                f.write("\nAverage Binary Recall: " + str(np.round(np.mean(rcs, axis=0)[i], 3)))
            ...
            for i, results in enumerate(result_data):
                ...
                f.write("\nMacro Loss: " + str(np.round(results[0]["loss"], 3)))
                f.write("\nMacro Accuracy: " + str(np.round(results[0]["accuracy"], 3)))
                f.write("\nMacro F1-Score: " + str(np.round(results[0]["f1-score"], 3)))
                f.write("\nMacro Precision: " + str(np.round(results[0]["precision"], 3)))
                f.write("\nMacro Recall: " + str(np.round(results[0]["recall"], 3)))
                f.write("\nAUC: " + str(np.round(results[0]["auc"], 3)))
                for label, result in results[1].items():
                    f.write("\n\n" + label + ':')
                    f.write("\nLoss: " + str(np.round(result["loss"], 3)))
                    f.write("\nAccuracy: " + str(np.round(result["accuracy"], 3)))
                    f.write("\nF1-score: " + str(np.round(result["f1-score"], 3)))
                    f.write("\nPrecision: " + str(np.round(result["precision"], 3)))
                    f.write("\nRecall: " + str(np.round(result["recall"], 3)))
                    f.write("\nConfusion Matrix: \n" + str(result["confusion_matrix"]))

    def test(self, X: np.ndarray, Y: np.ndarray, k=3, class_labels=None, test_set=None):
